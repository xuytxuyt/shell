...
# elements, nodes = import_roof_mix("msh/scordelislo_16.msh")
...
function Γ¹(x)
    x_ = Vec{3}((x[1],x[2],0.0))
    return Tensor{2,2}((Γ¹₁₁(x_),Γ¹₁₂(x_),Γ¹₁₂(x_),Γ¹₂₂(x_)))
end
function Γ²(x)
    x_ = Vec{3}((x[1],x[2],0.0))
    return Tensor{2,2}((Γ²₁₁(x_),Γ²₂₁(x_),Γ²₂₁(x_),Γ²₂₂(x_)))
...
#         # println(temp)
#         global temp1 += Γᵞᵧₐ(x_)[1]*𝑤
#         global temp2 += Γᵞᵧₐ(x_)[2]*𝑤
...
# for a in elements["Γₚ"]
    ...
    t¹ = (x₂-x₁)/𝐿
    t² = (y₂-y₁)/𝐿
    check = 𝐿 - a.𝐿; check ≉ 0.0 ? println("𝐿 wrong! $check") : nothing
    ...
    for ξ in 𝓖
        ...
        # global temp1 += (∂𝒔ₐ⋅Aᵃᵇ(x_)⋅𝒏ₐ(x_) + 𝒔ₐ(x_)⋅Aᵃᵇ(x_)⋅∂𝒏ₐ)*𝑤 + 𝑓(x_)*Mₛₙ(x_)*Δ + (Γ¹(x_) ⊡ Aᵃᵇ(x_)*n₁(x_) + Γ²(x_) ⊡ Aᵃᵇ(x_)*n₂(x_))*𝑤
        # global temp1 += (Γ¹(x_) ⊡ Aᵃᵇ(x_)*n₁(x_) + Γ²(x_) ⊡ Aᵃᵇ(x_)*n₂(x_))*𝑤
        # global temp1 += - 𝑓(x_)*∂Mₛₙ(x_)⋅𝒔ᵃ(x_)*𝑤 + 𝑓(x_)*Mₛₙ(x_)*Δ - 𝑓(x_)*Γᵞᵧₐ(x_)⋅Aᵃᵇ(x_)⋅𝒏ₐ(x_)*𝑤
        global temp1 += - 𝑓(x_)*∂Mₛₙ(x_)⋅𝒔ᵃ(x_)*𝑤 + 𝑓(x_)*Mₛₙ(x_)*Δ - 𝑓(x_)*Γᵞᵧₐ(x_)⋅Aᵃᵇ(x_)⋅𝒏ₐ(x_)*𝑤 - (Γ¹(x_) ⊡ Aᵃᵇ(x_)*n₁(x_) + Γ²(x_) ⊡ Aᵃᵇ(x_)*n₂(x_))*𝑤
        # println(∂Mₛₙ(x_))
        # println(Aᵃᵇ(x_))
        # println(Δ)
        # println(𝑓(x_))
...
for a in elements["Ω"]
    ...
    for ξ in 𝓖
        ...
        # global temp2 -= Γᵞₐᵦᵧ(x_) ⊡ Aᵃᵇ(x_)*𝑤 + (Γᵞᵧₐ(x_)[1]*(Γ¹(x_) ⊡ Aᵃᵇ(x_)) + Γᵞᵧₐ(x_)[2]*(Γ²(x_) ⊡ Aᵃᵇ(x_)))*𝑤
        global temp2 -=  Γᵞₐᵦᵧ(x_) ⊡ Aᵃᵇ(x_)*𝑤 + (Γᵞᵧₐ(x_)[1]*(Γ¹(x_) ⊡ Aᵃᵇ(x_)) + Γᵞᵧₐ(x_)[2]*(Γ²(x_) ⊡ Aᵃᵇ(x_)))*𝑤 + Γᵞᵧₐ(x_)⋅Aᵃᵇ(x_)⋅Γᵞᵧₐ(x_)*𝑤 + Γᵞᵧᵦₐ(x_) ⊡ Aᵃᵇ(x_)*𝑤 
    end
    # println(temp)
end
println(temp1)
